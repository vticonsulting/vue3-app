<template>
  <router-link
    to="/"
    class="flex items-center bg-white dark:bg-primary-900 text-primary-500 dark:text-white"
  >
    <div class="flex items-center justify-center shrink-0" style="width: 54px; height: 64px">
      <svg
        width="48"
        height="48"
        viewBox="0 0 48 48"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M25.1074 6C20.9532 7.42373 12.5157 10.3119 12 10.4746V18L25.1074 14.339L38 18V10.4746L25.1074 6Z"
          fill="currentColor"
          stroke="currentColor"
        />
        <path
          d="M20.1653 20L12 22.2373V42H20.1653V34.5424H29.8347V42H38V22.2373L29.8347 20V27.2712H20.1653V20Z"
          fill="currentColor"
          stroke="currentColor"
        />
      </svg>
    </div>
    <transition name="fade" appear mode="out-in">
      <hancock-ux-lab-logo />
    </transition>
  </router-link>
</template>
